MODULE gem_equil
  ...
  real :: mimp=1,mcmp=12,chgi=1,chgc=6
  ...
  integer :: nr=256,nr2=128,ntheta=128,isgnq=-1,idiag=0
...
contains
  subroutine new_equil()
      real :: pi,pi2,r,th,s,ss,c1,c2,lti,s0,s1,delsi,lte,delse,ln,deln,lnh,delsh,sh
      ...
      parameter(nraefl=513,nrgtc=513,nthgtc=513)
      real :: Rgtc(0:nrgtc-1,0:nthgtc-1),Zgtc(0:nrgtc-1,0:nthgtc-1),Bgtc(0:nrgtc-1,0:nthgtc-1),rhogtc(0:nrgtc-1),thgtc(0:nthgtc-1),rminorgtc(0:nrgtc-1)
...
!Normalization
      ...
      ! add f_raefl ni_reafl capni capne capti capte
      do i = 0,nraefl-1
         read(51,*)rhogtc(i),sf_raefl(i),f_raefl(i),ne_raefl(i),ni_raefl(i),te_raefl(i),ti_raefl(i),capni_raefl(i),capne_raefl(i),capti_raefl(i),capte_raefl(i)
      end do
      close(51)
...
! check nigtc, netgc, tigtc, tegtc, capnigtc, capnegtc, captigtc, captegtc
      do i = 0,nr
         ...
         fgtc(i) = wx0*f_raefl(k)+wx1*f_raefl(k+1)
         sfgtc(i) = wx0*sf_raefl(k)+wx1*sf_raefl(k+1)
         nigtc(i) = wx0*ni_raefl(k)+wx1*ni_raefl(k+1)  
         ...
         capnigtc(i) = wx0*capni_raefl(k)+wx1*capni_raefl(k+1)
         capnegtc(i) = wx0*capne_raefl(k)+wx1*capne_raefl(k+1)
         captigtc(i) = wx0*capti_raefl(k)+wx1*capti_raefl(k+1)
         captegtc(i) = wx0*capte_raefl(k)+wx1*capte_raefl(k+1)
...
! compute f
! going to add f directly, don't need the btor(i,j)
      !do i = 0,nr
      !   dum = 0.
      !   do j = 0,ntheta-1
...
! compute grad r,grad theta, grdgt,grcgt
      do i = 0,nr
         r = rin+i*dr
         !useless variables x, xp, elonp, triap
         !x = asin(tria(i))
         !xp = stria(i)/r
